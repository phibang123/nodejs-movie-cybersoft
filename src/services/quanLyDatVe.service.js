		...
		let [phongVeCustom] = [PhongVe].map((pv) => {
			return {
				thongTinPhim: {
					...
					tenPhim:
						pv?.phimChieuRap?.Phim === null
							? "PHIM ĐÃ BỊ XÓA"
							: pv?.phimChieuRap?.Phim?.P_tenPhim,
				...
				danhSachGhe: pv.gheLichChieu.map((ghe) => {
					return {
						...
						taiKhoanNguoiDat:
							ghe.thongTinVe == null
								? null
								: ghe.thongTinVe?.thongTinDatVe?.ND_taiKhoan,
...
const datVe = async (data) => {
	try {
		let { id, maLichChieu, danhSachVe } = data;
     
    
    
    let timLichChieu = await LichChieu.findOne({
      where: {
        LC_maLichChieu: maLichChieu
      }
    })
     
    if (maLichChieu[0] === null)
    {
      throw new Error("hãy nhập ghế")
    }
    if (!timLichChieu)
    {
      throw new Error("không tìm thấy lịch chiếu")
		...
		let mapDatVe = danhSachVe.map((dv) => {
			return dv.maGhe;
		});

		let datVeCot = await DatVe.create({ ND_id: id });

		await GheXuatChieu.update(
			{ DV_maVe: datVeCot.DV_maVe },
			{
				where: {
					G_maGhe: mapDatVe,
					LC_maLichChieu: maLichChieu,
